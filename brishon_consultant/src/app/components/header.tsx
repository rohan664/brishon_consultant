...
function Header(){
    ...
    return(
        <div className="w-full flex item-center justify-between p-4 rounded-b-[3rem] bg-white z-20 fixed bg shadow-lg shadow-grey-500/50">
            <p className="text-primary font-bold text-large px-4 cursor-pointer">Brishon.</p>
                ...
                <>
                  ...
                  <div className="bg-white rounded-lg absolute top-12 right-10">
                    <div className="flex flex-col gap-4 text-black font-semibold items-start cursor-pointer p-4">
                        <p className="text-base hover:text-primarylight ease-in-out duration-600" onClick={()=>{handleNavigation("/"),setIsSidebar(!isSidebar)}}>Home</p>
                        <p className="text-base hover:text-primarylight ease-in-out duration-600" onClick={()=>{handleNavigation("/document"),setIsSidebar(!isSidebar)}}>Documnet</p>
                        <p className="text-base hover:text-primarylight ease-in-out duration-600" onClick={()=>{handleNavigation("/about-us"),setIsSidebar(!isSidebar)}}>About Us</p>
                        <p className="text-base hover:text-primarylight ease-in-out duration-600" onClick={()=>{handleNavigation("/contact"),setIsSidebar(!isSidebar)}}>Contact Us</p>
                      </div>
                  </div>
                </>
              }
